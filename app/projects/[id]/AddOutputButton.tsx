        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-brand-surface border border-white/10 rounded-xl w-full max-w-xs p-6 shadow-2xl">
            ...
            <p className="text-xs text-white/40 mb-5">
              Adds <span className="font-mono text-white/60">{`${jobId}${type === 'episode' ? 'E' : 'H'}${num}`}</span>
            </p>
                ...
                <button
                  ...
                  className={`py-2.5 text-sm font-medium rounded border transition-colors ${
                    type === t
                      ...
                      : 'bg-brand-surface2 border-white/10 text-white/60 hover:text-white hover:bg-white/10'
                  }`}
                >
            ...
            {/* Number */}
            <label className="text-[10px] text-white/30 uppercase tracking-wider">Number</label>
            <select
              value={num}
              onChange={e => setNum(parseInt(e.target.value, 10))}
              className="w-full mt-1 bg-brand-surface2 border border-white/10 rounded px-3 py-2 text-sm text-white/80 focus:outline-none focus:border-brand-red/50"
            >
            ...
            <button
              onClick={add}
              disabled={loading}
              className="w-full mt-5 py-2.5 bg-brand-red hover:bg-brand-red-dim disabled:opacity-50 text-white text-sm font-medium rounded transition-colors"
            >
