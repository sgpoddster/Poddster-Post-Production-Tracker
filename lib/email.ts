...
// ── Footage delivery email ────────────────────────────────────────────────────
export async function sendFootageDeliveryEmail({
  toEmails,
  ccEmails = [],
  clientFirstName,
  clientName,
  ...
  if (to.length === 0) {
    console.warn(`[email] footage delivery: no email on file for ${clientName} — skipping`)
    return
  }
...
<body style="margin:0;padding:0;background:#0f0f1a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    ...
    <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
      ...
      <tr><td style="background:#1a1a2e;border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:36px;">
        ...
        <p style="margin:0 0 20px;font-size:15px;color:rgba(255,255,255,0.7);line-height:1.7;">
          It was a pleasure to host you at Poddster. Here is your footage.
  ...
  try {
    ...
    await sendViaGas(to.join(','), 'Your Poddster Footage is Ready', html, cc || undefined)
  } catch (e) {
    console.error('[email] footage delivery error:', e)
  }
}
