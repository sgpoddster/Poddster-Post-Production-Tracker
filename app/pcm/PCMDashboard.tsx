...
export default function PCMDashboard({ initialRecordings }: { initialRecordings: Recording[] }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'pcm_recordings' },
        (payload) => {
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-th/40">
            Synology NAS → Google Drive · Live
          </p>
        </div>
          ...
          >
            ...
            {latest ? (
              <>
                ...
                <div className="mt-2"><Badge state={latest.state} /></div>
                {active && (
      ...
      {failingCount > 0 && (
        <div className="mb-6 rounded-lg border border-red-500/20 bg-red-500/10 px-4 py-3">
          <p className="text-sm font-medium text-red-400">
            ⚠ {failingCount} recording{failingCount > 1 ? 's' : ''} failed — see table below.
          </p>
        </div>
      )}
