...
async function run(req: NextRequest) {
  ...
  for (const rows of Array.from(groups.values())) {
    ...
    const toEmails = [client?.email, client?.email_2, client?.email_3].filter((e): e is string => !!e)
    ...
    // Advance the chase stage on these projects regardless (so we don't re-chase).
    await supabase
      .from('projects')
  ...
  return NextResponse.json({ ok: true, due: due.length, emailsSent, clients: groups.size })
}
