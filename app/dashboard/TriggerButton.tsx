...
export default function TriggerButton({ projectId, isAdmin }: Props) {
  ...
  return (
    <div className="flex items-center gap-2 bg-brand-surface2 border border-white/10 rounded px-2 py-1.5">
      <div className="flex flex-col gap-0.5">
        <span className="text-[10px] text-white/30 leading-none">Submission date</span>
        <input
          type="date"
          value={submittedDate}
          max={todayStr()}
          onChange={e => setSubmittedDate(e.target.value)}
          className="bg-transparent text-xs text-white/80 focus:outline-none w-32"
        />
      </div>
      <button
