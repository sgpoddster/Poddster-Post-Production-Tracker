...
export function ClientFilter({ clients, selected }: { clients: string[]; selected: string | null }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <span className="max-w-[160px] truncate">{selected ?? 'All clients'}</span>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 z-50 w-[240px] rounded-lg border border-white/10 bg-[#1a1a2e] shadow-xl overflow-hidden">
          ...
          <div className="max-h-64 overflow-y-auto">
            <button
              onClick={() => pick(null)}
              className={`w-full text-left px-3 py-2 text-xs hover:bg-white/[0.06] transition-colors ${!selected ? 'text-white' : 'text-white/45'}`}
            >
              All clients
            </button>
            {filtered.map(c => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-sm hover:bg-white/[0.06] transition-colors truncate ${
                  selected === c ? 'text-white bg-white/[0.04]' : 'text-white/60'
                }`}
              >
                {c}
              </button>
