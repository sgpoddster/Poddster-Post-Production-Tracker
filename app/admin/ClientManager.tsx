...
function CopyPortalLink({ token }: { token: string }) {
  ...
  return (
    <button onClick={copy} className="text-xs text-th/25 hover:text-th/60 transition-colors">
...
function ClientEmails({ client }: { client: Client }) {
  ...
  if (!emails.length) return <span className="text-th/20 text-xs italic">No email</span>
  return (
    <div className="flex flex-col gap-0.5">
      {emails.map((e, i) => (
        <span key={i} className="text-xs text-th/40">{e}</span>
      ))}
    </div>
  )
}
...
function EditRow({ client, onSave, onCancel }: {
  ...
  return (
    <div className="px-5 py-4 space-y-3 bg-th/[0.02]">
      {/* Client name (code lives in the parentheses) */}
      <div className="space-y-1">
        <label className="text-[10px] text-th/30 uppercase tracking-wider">Client name *</label>
        <input value={form.name} onChange={set('name')} placeholder="e.g. Adam Fayed (4AF)"
          className="w-full bg-brand-surface2 border border-th/10 rounded px-3 py-1.5 text-sm text-th placeholder-th/25 focus:outline-none focus:border-th/25" />
      </div>
      {/* Row 2: First + Last name */}
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <label className="text-[10px] text-th/30 uppercase tracking-wider">First name</label>
          <input value={form.first_name} onChange={set('first_name')} placeholder="First name"
            className="w-full bg-brand-surface2 border border-th/10 rounded px-3 py-1.5 text-sm text-th placeholder-th/25 focus:outline-none focus:border-th/25" />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] text-th/30 uppercase tracking-wider">Last name</label>
      ...
      <div className="space-y-2">
        <label className="text-[10px] text-th/30 uppercase tracking-wider">Email addresses</label>
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none pt-1">
        ...
        <span className="text-xs text-th/60">Exclude from reminder emails</span>
      </label>
      <div className="flex items-center gap-2 pt-1">
        <button onClick={save} disabled={loading || !form.name.trim()}
          className="px-4 py-1.5 bg-brand-red hover:bg-brand-red-dim disabled:opacity-50 text-th text-xs font-medium rounded transition-colors">
          {loading ? 'Saving…' : 'Save'}
        </button>
...
export default function ClientManager({ initialClients }: { initialClients: Client[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-brand-surface2 border border-th/10 rounded-lg pl-8 pr-7 py-1.5 text-sm text-th placeholder-th/25 focus:outline-none focus:border-th/25"
          />
          {query && (
            <button onClick={() => setQuery('')}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-th/30 hover:text-th/60 text-sm leading-none">✕</button>
          )}
        </div>
        {!adding && (
          <button onClick={() => setAdding(true)}
            className="shrink-0 px-3 py-1.5 bg-brand-red hover:bg-brand-red-dim text-th text-xs font-medium rounded-lg transition-colors">
            + Add client
          </button>
        )}
      </div>
      ...
      {adding && (
        <form onSubmit={addClient} className="rounded-lg border border-th/[0.08] bg-brand-surface p-4 space-y-3">
          <p className="text-xs font-semibold text-th/50 uppercase tracking-wider">New client</p>
          <div className="space-y-1">
            <label className="text-[10px] text-th/30 uppercase tracking-wider">Client name *</label>
            <input value={newForm.name} onChange={setNew('name')} placeholder="e.g. Benjamin Loh (QW2)" required
              className="w-full bg-brand-surface2 border border-th/10 rounded px-3 py-1.5 text-sm text-th placeholder-th/25 focus:outline-none focus:border-th/25" />
            <p className="text-[10px] text-th/25">Include the code in brackets — it's picked up automatically.</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              <label className="text-[10px] text-th/30 uppercase tracking-wider">First name</label>
      ...
      {q && (
        <p className="text-xs text-th/30 px-1">
      ...
      <div className="rounded-lg border border-th/[0.06] bg-brand-surface overflow-hidden divide-y divide-th/[0.06]">
        {clients.length === 0 && (
          <p className="px-5 py-4 text-sm text-th/25">No clients yet.</p>
        )}
        {clients.length > 0 && filtered.length === 0 && (
          <p className="px-5 py-4 text-sm text-th/25">No clients match &ldquo;{query.trim()}&rdquo;</p>
        )}
        {filtered.map(c => (
          <div key={c.id}>
            {editingId === c.id ? (
              ...
              <div className="flex items-center justify-between px-5 py-3.5 hover:bg-th/[0.02] transition-colors">
                <div className="flex items-center gap-4 min-w-0">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-th">{c.name}</span>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button onClick={() => setEditingId(c.id)}
                    className="text-xs text-th/30 hover:text-th/60 transition-colors">
                    Edit
                  </button>
                  <button onClick={() => deleteClient(c.id)}
                    className="text-xs text-th/25 hover:text-brand-red transition-colors">
                    Remove
                  </button>
                </div>
              </div>
